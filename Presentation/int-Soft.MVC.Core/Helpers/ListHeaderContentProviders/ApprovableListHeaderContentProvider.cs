...
{
    public class ApprovableListHeaderContentProvider : IListHeaderContentProvider
    {
        public MvcHtmlString GetListHeaderContent(IntSoftExtension helper)
        {
            return helper.Div("row", new MvcHtmlString(string.Format("{0} {1}",
                helper.Div("col-md-6", helper.Button(ButtonSetting.CreateButtonSetting())),
                helper.Div("col-md-6", helper.Button(ButtonSetting.DraftButtonSetting())))));
        }
    }
}